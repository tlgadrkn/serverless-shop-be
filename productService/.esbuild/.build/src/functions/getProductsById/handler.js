...
var formatJSONResponse = (response, statusCode) => {
  return {
    headers,
...
var api_gateway_default = createErrorResponse;
...
var httpJsonBodyParserMiddleware = (opts = {}) => {
  ...
  const httpJsonBodyParserMiddlewareBefore = async (request) => {
    const { headers: headers2, body } = request.event;
    const contentType = headers2["Content-Type"] ?? headers2["content-type"];
...
var docClient = new import_dynamodb.default.DocumentClient();
var getProductsById = async (event) => {
  ...
  try {
    ...
    if (!productData.Items[0]) {
      return api_gateway_default(404, `Product with ${id} not found.`);
    }
    const stockData = await docClient.query({
      TableName: process.env.STOCKS_TABLE,
      KeyConditionExpression: "product_id = :product_id",
      ExpressionAttributeValues: { ":product_id": id }
    }).promise();
    const data = { ...productData.Items[0], count: (_a = stockData == null ? void 0 : stockData.Items[0]) == null ? void 0 : _a.count };
